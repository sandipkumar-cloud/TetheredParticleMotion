...
for z=1:size(e.x,2)
    d.x_cor(1:size(e.x,1),z)=e.x(1:size(e.x,1),z)-d.x_mean(1:size(e.x,1),z);
    d.y_cor(1:size(e.x,1),z)=e.y(1:size(e.x,1),z)-d.y_mean(1:size(e.x,1),z);
    d.x_square(1:size(e.x,1),z)=d.x_cor(1:size(e.x,1),z).^2;
    d.y_square(1:size(e.x,1),z)=d.y_cor(1:size(e.x,1),z).^2;
    d.rho_square(1:size(e.x,1),z)=(d.x_cor(1:size(e.x,1),z).^2)+(d.y_cor(1:size(e.x,1),z).^2);
    d.rho(1:size(e.x,1),z)=d.rho_square(1:size(e.x,1),z).^0.5;
end
...
i=1;
d.bins_rho_avg(i)=100;
while d.bins_rho_avg(i)<350
