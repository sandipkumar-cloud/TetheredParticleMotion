...
for j=1:length(b)
    if isempty(b(1,j).OK.indices)==0
        for k=1:size(b(1,j).OK.beadsx,2)
            m=m+1;
            for l=1:size(b(1,j).OK.beadsx,1)
                e.ALL.rho_overall(l,m)=sqrt((b(1,j).OK.beadsx(l,k)^2)+(b(1,j).OK.beadsy(l,k))^2);
                e.ALL.x(l,m)=b(1,j).OK.beadsx(l,k);
                e.ALL.y(l,m)=b(1,j).OK.beadsy(l,k);
%                 e.ALL.time(l,m)=b(1,j).OK.time(l);
            end
            e.ALL.length(m)=size(b(1,j).OK.beadsx,1);
...
for j=1:length(b)
    if isempty(b(1,j).REC.indices)==0
        for k=1:size(b(1,j).REC.beadsx,2)
            m=m+1;
            for l=1:b(1,j).REC.length(k)
                e.ALL.rho_overall(l,m)=sqrt((b(1,j).REC.beadsx(l,k)^2)+(b(1,j).REC.beadsy(l,k))^2);
                e.ALL.x(l,m)=b(1,j).REC.beadsx(l,k);
                e.ALL.y(l,m)=b(1,j).REC.beadsy(l,k);
%                 e.ALL.time(l,m)=b(1,j).REC.time(l,k);
            end
